...
train_data = Bindataset(train_bin, GPTConfig.block_size)
valid_data = Bindataset(valid_bin, GPTConfig.block_size)
...
def estimate_loss():
    ...
    for _ in range(eval_iters):
        xb, yb = valid_data.get_batch(batch_size)
        _, loss = model(xb, yb)
...
def save_checkpoint(step):
    torch.save(
        ...
        "TinyStoriesV2-GPT4-model.pt",
    )

tokens_per_step = batch_size * GPTConfig.block_size
...
print("Training complete. Model saved to TinyStoriesV2-GPT4-model.pt")